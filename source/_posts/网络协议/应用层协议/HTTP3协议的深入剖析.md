...
![TLS3](https://img.hellobyebye.com/doc/2021032422272516165960451616596045106y5pyES.png)
...
![Long Packet Header](https://img.hellobyebye.com/doc/202103242229291616596169161659616955077G4JU.png)
...
![Packet Header](https://img.hellobyebye.com/doc/2021032422300816165962081616596208029xctCuv.png)
...
![Long Header](https://img.hellobyebye.com/doc/2021032422305016165962501616596250352oIy5eq.png)

其中，显示为E（Encrypt）的字段表示被TLS加密过。当然，Packet Header只是描述了最基本的连接信息，其上的Stream层、HTTP消息也是被加密保护的：

![Encrypt](https://img.hellobyebye.com/doc/2021032422315916165963191616596319830SsAQ5Q.png)

现在我们已经对HTTP3协议的格式有了基本的了解，接下来我们通过队头阻塞问题，看看Packet之上的QUIC Frame、HTTP3 Frame帧格式。

# Stream多路复用时的队头阻塞问题解决

其实，解决队头阻塞的方案，就是允许微观上有序发出的Packet报文，在接收端无序到达后也可以应用于并发请求中。比如上文的动态图中，如果丢失的黄色报文对其后发出的绿色报文不造成影响，队头阻塞问题自然就得到了解决：

![对头阻塞问题的解决](https://img.hellobyebye.com/doc/20210324223300161659638016165963806320YzB88.gif)
...
![QUIC](https://img.hellobyebye.com/doc/20210324223544161659654416165965441392qhP3u.png)

每一个Frame都有明确的类型：

![Frame类型](https://img.hellobyebye.com/doc/2021032422361516165965751616596575656IMsgUl.png)
...
![Stream Frame](https://img.hellobyebye.com/doc/2021032422372516165966451616596645524GRsGBI.png)
...
![Stream](https://img.hellobyebye.com/doc/2021032422382516165967051616596705333bEOMci.png)
...
![静态表](https://img.hellobyebye.com/doc/2021032422402616165968261616596826684CQVW2x.png)

而在QPACK中，则上升为98个静态表项，比如Nginx上的ngx_htt_v3_static_table数组所示

![QPACK](https://img.hellobyebye.com/doc/2021032422405716165968571616596857872vo5BpS.png)
...
![Stream Type](https://img.hellobyebye.com/doc/2021032422412616165968861616596886163iOEunh.png)

因此，当Stream ID是0、4、8、12时，这就是客户端发起的双向Stream（HTTP3不支持服务器发起双向Stream），它用于传输HTTP请求与响应。单向Stream有很多用途，所以它在数据前又多出一个Stream Type字段：

![StreamId](https://img.hellobyebye.com/doc/2021032422420216165969221616596922060c7PK5i.png)
...
> **原文作者：** 陶辉 
> **原文链接：** https://www.taohui.tech/2021/02/04/网络协议/深入剖析HTTP3协议/